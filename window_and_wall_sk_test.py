# -*- coding: utf-8 -*-
"""
window + wall thinning test
"""
import cv2
import os

def thinning(filepath , fpnumber, outputpath):
    ...
    ret, image = cv2.threshold(gray,30,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    ...
    def pre_thinning1(image):
        ...
        print("pre_thinning is finished!")
        return image
    ...
    cv2.imwrite(fpnumber + '_thin.bmp' , BW_Skeleton)
    return BW_Skeleton


filepath = 'C:/Users/user/Desktop/ww_separated'
...
for file in files:
    fp_file = cv2.imread(filepath + '/' + file)
    print(file) 
    print("thinning starts!")
    a = thinning(filepath, file, outputpath)
